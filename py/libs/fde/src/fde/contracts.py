"""Official FDEBench task contracts (canonical source of truth).

These Pydantic models match the JSON schemas in ``py/data/task{1,2,3}/`` and the
required response keys validated by the platform's preflight check. They live in the
library so the service layer and the transport app share one definition; the app's
``models`` module re-exports from here.

Do not redefine these enums elsewhere — import them from ``fde.contracts``.
"""
