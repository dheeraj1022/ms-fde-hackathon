...
[`openapi.yaml`](../../openapi.yaml) (repo root) is the **design source of truth** for the
deployed service. It documents all four endpoints — `GET /health`, `POST /triage`,
`POST /extract`, `POST /orchestrate` — with their exact request/response schemas (mirroring
`py/apps/sample/models.py`), enum vocabularies, response headers, error envelopes, and worked
examples.
